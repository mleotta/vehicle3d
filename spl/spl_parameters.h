// This is spl/spl_parameters.h
#ifndef spl_parameters_h_
#define spl_parameters_h_
//:
// \file
// \brief classes to represent parameters to spl processes
...
class spl_param
{
 public:
  ...
  virtual spl_param * clone() const = 0;
 ...
 protected:
  //: Constructor
  spl_param(bool has_bounds, const vcl_string& name, const vcl_string& desc)
...
template< class T >
class spl_param_type : public spl_param
{
 public:
  // Constructor - with bounds
  spl_param_type<T>(const vcl_string& name, const vcl_string& desc, const T& dflt, const T& min, const T& max)
   : spl_param(true, name, desc), value_(dflt), default_(dflt), temp_value_(dflt),
     min_value_(min), max_value_(max) { assert( value_ < max_value_ &&  min_value_ < value_ ); }

  // Constructor - without bounds
  spl_param_type<T>(const vcl_string& name, const vcl_string& desc, const T& dflt)
   : spl_param(false, name, desc), value_(dflt), default_(dflt), temp_value_(dflt),
  ...
  //: Clone the parameter
  virtual spl_param * clone() const { return new spl_param_type<T>(*this); }
...
class spl_parameters : public vbl_ref_count
{
 public:
  ...
  spl_parameters( const spl_parameters_sptr& old_params);
  ...
  vcl_vector< spl_param* > get_param_list() const;
 ...
 private:
  ...
  bool add( spl_param* param );

  template<class T>
  bool get_param( const vcl_string& name, 
                  spl_param_type<T> * &param) const
  {
    vcl_map< vcl_string, spl_param* >::const_iterator 
      itr = name_param_map_.find( name );
    if( itr == name_param_map_.end() ) {
    ...
    param = dynamic_cast<spl_param_type<T> *>(itr->second);
    if( !param )
  ...
  vcl_map< vcl_string , spl_param* > name_param_map_;
  //: The vector of parameters in order of declaration
  vcl_vector< spl_param* > param_list_;
};
...
class spl_filepath
{
 public:
  //: Constructor
  spl_filepath(const vcl_string& p = "", const vcl_string& e = "*")
   : path(p), ext(e) {}
