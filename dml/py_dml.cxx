// This is dml/py_dml.cxx
//:
// \file
// \brief The python interface for dml
...
set_camera(PyObject *self, PyObject *args)
{
  ...
  bool ret = py_dml_manager::instance()->set_camera(frame,filename);
...
set_image(PyObject *self, PyObject *args)
{
  ...
  bool ret = py_dml_manager::instance()->set_image(frame,filename);
...
set_video(PyObject *self, PyObject *args)
{
  ...
  bool ret = py_dml_manager::instance()->set_video(filename);
...
{
  py_dml_manager::instance()->reset_views();
...
set_parts(PyObject *self, PyObject *args)
{
  ...
  bool ret = py_dml_manager::instance()->set_parts(filename);
...
set_pca(PyObject *self, PyObject *args)
{
  ...
  bool ret = py_dml_manager::instance()->set_pca(filename);
...
set_vehicle_model(PyObject *self, PyObject *args)
{
  ...
  if(vcl_string(type) == "Detailed1")
    py_dml_manager::instance()->set_vehicle_model(py_dml_manager::DETAILED1);
    ...
    py_dml_manager::instance()->set_vehicle_model(py_dml_manager::DETAILED3);
  else if(vcl_string(type) == "Dodecahedral")
    py_dml_manager::instance()->set_vehicle_model(py_dml_manager::DODECAHEDRAL);
  else if(vcl_string(type) == "Ferryman")
    py_dml_manager::instance()->set_vehicle_model(py_dml_manager::FERRYMAN);
  else
...
set_fit_mode(PyObject *self, PyObject *args)
{
  ...
  if(vcl_string(mode) == "video")
    py_dml_manager::instance()->set_fit_mode(true);
  else if(vcl_string(mode) == "multiview")
    py_dml_manager::instance()->set_fit_mode(false);
  else
    return Py_BuildValue("b", false);
...
set_options(PyObject *self, PyObject *args)
{
  ...
  py_dml_manager::instance()->set_options(options,num_pc);
...
{  
  ...
  if (!PyArg_ParseTuple(args, "d:set_init_uncert", &uncert))
    return NULL;
  py_dml_manager::instance()->set_init_uncert(uncert);
...
set_translation(PyObject *self, PyObject *args)
{
  ...
  py_dml_manager::instance()->set_translation(tx, ty, tz);
...
set_params(PyObject *self, PyObject *args)
{
  ...
  py_dml_manager::instance()->set_params(params);
...
{  
  ...
  if (!PyArg_ParseTuple(args, "d:set_lambda", &lambda))
    return NULL;
  py_dml_manager::instance()->set_lambda(lambda);
...
set_truth_mesh(PyObject *self, PyObject *args)
{
  ...
  bool ret = py_dml_manager::instance()->set_truth_mesh(filename);
...
{  
  ...
  double error = py_dml_manager::instance()->evaluate_residual(scale,compute_vis);
...
{  
  double error = py_dml_manager::instance()->compute_error();
...
{  
  ...
  if (!PyArg_ParseTuple(args, "|i:fit_model", &max_itr))
    return NULL;
  py_dml_manager::instance()->fit_model(max_itr);
...
write_svg_curves(PyObject *self, PyObject *args)
{
  ...
  bool ret = py_dml_manager::instance()->write_svg_curves(frame,filename);
...
{  
  ...
  if (!PyArg_ParseTuple(args, "b:enable_tracking", &enabled))
    return NULL;
  py_dml_manager::instance()->enable_tracking(enabled);
...
{  
  ...
  if (!PyArg_ParseTuple(args, "b:set_track_with_silhouette", &val))
    return NULL;
  py_dml_manager::instance()->set_track_with_silhouette(val);
...
{  
  ...
  if (!PyArg_ParseTuple(args, "|i:video_seek", &frame))
    return NULL;
  py_dml_manager::instance()->video_seek(frame);
...
{  
  bool ret = py_dml_manager::instance()->advance_video();
  return Py_BuildValue("b", ret);
}
...
{  
  int ret = py_dml_manager::instance()->current_frame();
...
{
  double tx,ty,tz;
  py_dml_manager::instance()->get_translation(tx,ty,tz);
...
get_params(PyObject *self, PyObject *args)
{
  ...
  py_dml_manager::instance()->get_params(params);
  unsigned num_params = py_dml_manager::instance()->num_pc();
...
{
  vcl_string model;
  py_dml_manager::vehicle_model vm;
  py_dml_manager::instance()->get_vehicle_model(vm);
  switch(vm){
    case py_dml_manager::DODECAHEDRAL:
      model = "Dodecahedral";
      break;
    case py_dml_manager::FERRYMAN:
      model = "Ferryman";
      break;
    case py_dml_manager::DETAILED1:
      model = "Detailed1";
      break;
    case py_dml_manager::DETAILED2:
      model = "Detailed2";
      break;
    case py_dml_manager::DETAILED3:
      model = "Detailed3";
      break;
  }
...
get_vehicle_states(PyObject *self, PyObject *args)
{
  const vcl_vector<dml_vehicle_state>& states =
      py_dml_manager::instance()->get_vehicle_states();
  
  PyObject* list = PyList_New(states.size());
  for(unsigned int i=0; i<states.size(); ++i)
  {
    const dml_vehicle_state& s = states[i];
...
compute_edgel_coverage(PyObject *self, PyObject *args)
{
  ...
  py_dml_manager::instance()->compute_edgel_coverage(dist_thresh,ncm,nct,npm,npt);
...
relative_coverage(PyObject *self, PyObject *args)
{
  ...
  py_dml_manager::instance()->compare_relative_coverage(dist_thresh,nm,np,nt);
...
static PyMethodDef py_dml_methods[] = {
...
initpy_dml(void)
{
  Py_InitModule("py_dml", py_dml_methods);
}
