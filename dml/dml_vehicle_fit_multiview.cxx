// This is dml/dml_vehicle_fit_multiview.cxx
//=========================================================================
//:
// \file
...
#include "dml_vehicle_fit_multiview.h"




//: Constructor
dml_vehicle_fit_multiview::dml_vehicle_fit_multiview()
...
void dml_vehicle_fit_multiview::reset()
{
  views_.clear();
  views_.resize(1);
}
...
void dml_vehicle_fit_multiview::
set_edge_map(unsigned int idx,
...
void dml_vehicle_fit_multiview::set_active(unsigned int idx, bool active)
{
  if(idx >= views_.size()){
    views_.resize(idx+1);
  }
...
dml_vehicle_fit_multiview::edge_map(unsigned int idx) const
{ 
  if(idx >= views_.size())
...
dml_vehicle_fit_multiview::camera(unsigned int idx) const
{ 
  if(idx >= views_.size())
...
dml_vehicle_fit_multiview::projector(unsigned int idx) const
{ 
  if(idx >= views_.size())
...
bool dml_vehicle_fit_multiview::is_active(unsigned int idx) const
{
  if(idx >= views_.size())
    return false;
...
void dml_vehicle_fit_multiview::
detect_and_set_edges(unsigned int idx,
                      ...
                      bool compute_visiblity)
{
  for(unsigned int idx=0; idx<views_.size(); ++idx)
  {
    if(!views_[idx].active)
      continue;
    dml_pca_vehicle_projector& mesh_projector = views_[idx].projector;
...
dml_vehicle_fit_multiview::view::view()
...
double dml_vehicle_fit_multiview::
                       ...
                       const vnl_vector<double>& sigma)
{
  ...
  for(unsigned int idx=0; idx<views_.size(); ++idx)
  {
    ...
    views_[idx].rel_scale = dml_vehicle_fit::estimate_initial_scale(sigma,J);
