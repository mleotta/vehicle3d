// This is dml/dml_vehicle_state.cxx
//=========================================================================
//:
// \file
...
unsigned int dml_vehicle_state::next_id = 0;


//: stack shape, pose, and velocity into a single state vector
vnl_vector<double> 
dml_vehicle_state::state_vector() const
{
...
void 
dml_vehicle_state::set_state(const vnl_vector<double>& s)
{
...
void dml_vehicle_state::init_covar(bool estimate_shape)
{
  unsigned int num = 8;
  if(estimate_shape)
    num += params.size();
...
dml_vehicle_state 
dml_circ_motion_predict(const dml_vehicle_state& s, double t)
{
  dml_vehicle_state st;
...
dml_circ_motion_predict_position(const dml_vehicle_state& s, double t)
{
  const double& va = s.a_velocity;
  const double& vt = s.t_velocity;
...
unsigned int
dml_read_track_file(const vcl_string& filename, 
