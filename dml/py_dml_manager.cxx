// This is dml/py_dml_manager.cxx
#include "py_dml_manager.h"
...
py_dml_manager *py_dml_manager::instance_ = 0;

//: The singleton pattern - only one instance of the manager can occur
py_dml_manager *py_dml_manager::instance()
{
  if (!instance_)
  {
    instance_ = new py_dml_manager();
  }
  return py_dml_manager::instance_;
}
...
void py_dml_manager::init_mesh_tex(dml_pca_vehicle& mesh)
{
...
bool py_dml_manager::set_camera(unsigned int frame, 
...
void py_dml_manager::compute_sun_direction()
{
...
bool py_dml_manager::set_image(unsigned int frame, const vcl_string& filename)
{
...
bool py_dml_manager::set_video(const vcl_string& filename)
{
...
void py_dml_manager::reset_views()
{
...
bool py_dml_manager::set_parts(const vcl_string& filename)
{
...
bool py_dml_manager::set_pca(const vcl_string& filename)
{
...
bool py_dml_manager::set_mesh(const vcl_string& filename, 
                                 const vcl_string& partsfile)
{
  ...
  if(!mesh_->params().empty())
  {
    if(partsfile == "")
      ...
      mesh_->set_params(mesh_->project(mesh_->vertices(),dml_read_vehicle_parts(partsfile)));
  }
...
void py_dml_manager::set_vehicle_model(vehicle_model vm)
{
  switch(vm){
    case DODECAHEDRAL:
      mesh_ = &dodec_mesh_;
...
bool py_dml_manager::set_poly_mesh(const vcl_string& filename)
{
...
void py_dml_manager::set_options(const vcl_vector<bool>& options,
                                 unsigned int num_pc)
{
  vcl_cout << "Enable ";
...
void py_dml_manager::set_init_uncert(double uncert)
{
...
void py_dml_manager::set_track_with_silhouette(bool val)
{
...
void py_dml_manager::set_translation(double tx, double ty, double tz)
{
  t_.set(tx,ty,tz);
...
void py_dml_manager::set_rotation(double rx, double ry, double rz)
{
...
//: set the rotation vector
void py_dml_manager::set_params(const vcl_vector<double>& params)
{
...
void py_dml_manager::set_lambda(double lambda)
{
...
void py_dml_manager::set_tracking_states(const vcl_vector<dml_vehicle_state>& states)
{
...
void py_dml_manager::set_fit_mode(bool use_video)
{
  if(use_video)
  {
...
bool py_dml_manager::is_fit_mode_video() const
{
...
void py_dml_manager::fit_model(unsigned int max_itr)
{
...
bool py_dml_manager::write_svg_curves(unsigned int i,
                                      const vcl_string& filename) const
{
  const dml_pca_vehicle_projector& projector = is_fit_mode_video() ? 
                                               video_optimizer_.projector() :
                                               mv_optimizer_.projector(i);
  return dml_write_svg_curves(filename, projector);
}


//: load a mesh as the ground truth mesh
bool py_dml_manager::set_truth_mesh(const vcl_string& filename)
{
...
double py_dml_manager::compute_error() const
{
...
unsigned int py_dml_manager::num_pc() const
{
...
void py_dml_manager::enable_tracking(bool enable_track)
{
...
void py_dml_manager::video_seek(int fnum)
{
  if(istream_)
...
int py_dml_manager::current_frame() const
{
  if(!istream_)
    return -1;
...
double py_dml_manager::evaluate_residual(double scale, bool compute_visibility)
{
...
void py_dml_manager::get_translation(double& tx, double& ty, double& tz) const
{
  tx = t_.x();
  ty = t_.y();
  tz = t_.z();
...
void py_dml_manager::get_rotation(double& rx, double& ry, double& rz) const
{
...
void py_dml_manager::get_params(vcl_vector<double>& params) const
{
...
void py_dml_manager::get_vehicle_model(vehicle_model& vm)
{
...
const vcl_vector<dml_vehicle_state>& 
py_dml_manager::get_vehicle_states() const
{
                          ...
                          unsigned int& num_total)
{
  ...
  for(unsigned int i=0; i<mv_optimizer_.num_views(); ++i)
  {
    ...
    dml_pca_vehicle_projector projector(ni,nj);
