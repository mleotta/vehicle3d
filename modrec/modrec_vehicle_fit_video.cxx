// This is mleotta/gui/pca_vehicle/modrec_vehicle_fit_video.cxx
//=========================================================================
//:
// \file
...
class modrec_edgemap_observer: public dbpro_observer
{
public:
  modrec_edgemap_observer(modrec_vehicle_fit_video& opt)
  : optimizer(opt) {}
  ...
  modrec_vehicle_fit_video& optimizer;
};
...
//: Constructor
modrec_vehicle_fit_video::modrec_vehicle_fit_video()
: sun_dir_(0.0,0.0,0.0), tracker_(this), track_with_silhouette_(false)
{
  ...
  tracker_.add_edgemap_observer(new modrec_edgemap_observer(*this));
}


//: clear all the data and reset to the initial single frame setup
void modrec_vehicle_fit_video::reset()
{
...
//: Assign an edge map
void modrec_vehicle_fit_video::set_edge_map(const vil_image_view<float>& edge_map)
{
  edge_map_ = edge_map;
...
void modrec_vehicle_fit_video::set_silhouette(const vgl_polygon<double>& sil)
{
  silhouette_ = sil;
}
...
//: Assign a camera 
void modrec_vehicle_fit_video::set_camera(const vpgl_perspective_camera<double>& camera)
{
  camera_ = camera;
...
void modrec_vehicle_fit_video::set_sun_direction(const vgl_vector_3d<double>& sun_dir)
{
  sun_dir_ = sun_dir;
...
void modrec_vehicle_fit_video::set_istream(const vidl_istream_sptr& istream)
{
...
void modrec_vehicle_fit_video::set_track_with_silhouette(bool val)
{
...
set_vehicle_model(const modrec_pca_vehicle& vehicle)
{
  vehicle_ = vehicle;
...
modrec_vehicle_fit_video::edge_map() const
{ 
  return edge_map_;
}
...
modrec_vehicle_fit_video::silhouette() const
{
  return silhouette_;
}
...
modrec_vehicle_fit_video::camera() const
{ 
  return camera_; 
}
...
modrec_vehicle_fit_video::projector() const
{ 
  return projector_; 
}
...
double modrec_vehicle_fit_video::estimate_initial_scale(const modrec_pca_vehicle& mesh,
                                                        const vgl_vector_3d<double>& translation,
                                                        const vgl_rotation_3d<double>& rotation,
...
double modrec_vehicle_fit_video::
               ...
               vgl_rotation_3d<double>& rotation)
{
  ...
  modrec_vehicle_fit::apply_solution(soln,mesh,translation,rotation);
...
vnl_matrix<double> modrec_vehicle_fit_video::
...
void modrec_vehicle_fit_video::
set_priors_from_track(const modrec_vehicle_state& state)
{
  unsigned int tvi = num_params_;
  unsigned int avi = num_params_+1;
...
void modrec_vehicle_fit_video::
update_posterior_covariance(const modrec_pca_vehicle& mesh,
...
bool
modrec_vehicle_fit_video::correct_state(modrec_vehicle_state& state)
{
