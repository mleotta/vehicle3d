...
//: Constructor
vpro_delay::vpro_delay(unsigned int size) 
 : max_size_(size),
   recieved_timestamp_(0), 
   recieved_signal_(VPRO_WAIT) 
{
}


//: Destructor
vpro_delay::~vpro_delay()
{
}
...
//: Runs the filter
vpro_signal
vpro_delay::run(unsigned long timestamp,
                 vpro_debug_observer* const debug)
{
  ...
  if(timestamp > this->timestamp_){
    ...
    if(this->last_signal_ == VPRO_WAIT)
    ...
    if(!this->notify_observers(VPRO_WAIT))
      this->last_signal_ = VPRO_WAIT;
...
void 
vpro_delay::shift_queue(unsigned long timestamp,
                         vpro_debug_observer* const debug)
{
  queue_mutex_.lock();
  this->recieved_signal_ = this->request_inputs(timestamp,debug);
  if( this->recieved_signal_ == VPRO_VALID){
    vpro_connector_sptr connector = input_connectors_.begin()->second;
