...
vpro_signal vpro_process::last_signal() const 
{ 
...
//: Runs the filter
vpro_signal
vpro_filter::run(unsigned long timestamp,
                  vpro_debug_observer* const debug)
{
  ...
  if(timestamp > this->timestamp_){
    ...
    if(this->last_signal_ == VPRO_VALID){
      if (debug){
...
//: Runs the filter
vpro_signal
vpro_source::run(unsigned long timestamp,
                  vpro_debug_observer* const debug)
{
  // notify the debugger if available
  if (debug) debug->notify_enter(this, timestamp);
...
//: Runs the filter
vpro_signal
vpro_sink::run(unsigned long timestamp,
                vpro_debug_observer* const debug)
{
  // notify the debugger if available
  if (debug) debug->notify_enter(this, timestamp);
...
//: Constructor
vpro_process::vpro_process() 
  : timestamp_(0), input_request_active_(false),
    last_signal_(VPRO_INVALID)
{
}
...
vpro_process::~vpro_process()
{
  ...
  typedef vcl_map<unsigned int, vcl_set<vpro_observer_sptr> >::iterator OItr;
  for(OItr obs = output_observers_.begin(); 
      obs != output_observers_.end(); ++obs)
  {
    for( vcl_set<vpro_observer_sptr>::iterator ob = obs->second.begin();
...
namespace{
  struct pthread_data
  {
    vpro_connector_sptr connector;
    unsigned long timestamp;
    vpro_debug_observer* debug;
    pthread_t thread;
  };
  ...
  void* pthread_launcher(void *arg)
  {
    ...
    vpro_signal s = pd->connector->request_data(pd->timestamp,pd->debug);
...
vpro_signal
vpro_process::request_inputs(unsigned long timestamp,
                              vpro_debug_observer* const debug)
{
  ...
  {
    delete pd[0].debug;
    vpro_signal s = pd[0].connector->request_data(timestamp,debug);
    if(s != VPRO_VALID)
      retval = s;
    vpro_signal m = pd[0].connector->data()->info();
    if(m != VPRO_VALID){
      retval = m;
    }
  }
  else // use threads
  {
    ...
    for(unsigned int i=0; i<pd.size(); ++i)
    {
      ...
      vpro_signal s = reinterpret_cast<vpro_signal&> (status);
      if(rc)
      {
      ...
      if(s != VPRO_VALID)
        retval = s;
      vpro_signal m = pd[i].connector->data()->info();
      if(m != VPRO_VALID)
        retval = m;
    }
  }
  ...
  for(Itr i = input_connectors_.begin(); i != input_connectors_.end(); ++i)
  {
    vpro_connector_sptr connector = i->second;
    if(!connector->data()){
      vpro_signal s = connector->request_data(timestamp,debug);
      if(s != VPRO_VALID)
        retval = s;
      if(s == VPRO_INVALID)
        break;
...
//: Request any missing inputs
vpro_signal
vpro_process::request_inputs_serial(unsigned long timestamp,
                              vpro_debug_observer* const debug)
{
  if(this->input_request_active_){
    vcl_cerr << "Warning detected cycle in data flow" <<vcl_endl;
...
bool
vpro_process::notify_observers(vpro_signal message)
{
  typedef vcl_map<unsigned int, vpro_storage_sptr >::const_iterator DItr;
  typedef vcl_map<unsigned int, vcl_set<vpro_observer_sptr> >::iterator OItr;
  // Do not send the message VPRO_VALID unless the data is valid
  if(message == VPRO_VALID)
    message = VPRO_INVALID;
  bool retval = true;
...
void
vpro_process::connect_input(unsigned int in_idx,
...
void
vpro_process::disconnect_input(unsigned int in_idx)
{
  typedef vcl_map<unsigned int, vpro_connector_sptr >::iterator Itr;
  Itr result = input_connectors_.find(in_idx);
  if(result != input_connectors_.end()){
...
void
vpro_process::add_output_observer(unsigned int idx, const vpro_observer_sptr& obs)
{
...
bool
vpro_process::remove_output_observer(unsigned int idx, const vpro_observer_sptr& obs)
{
  typedef vcl_map<unsigned int, vcl_set<vpro_observer_sptr> >::iterator Itr;
...
vpro_storage_sptr
vpro_process::get_input(unsigned int idx) const
{
  typedef vcl_map<unsigned int, vpro_connector_sptr>::const_iterator Itr;
...
const vcl_type_info&
vpro_process::input_type_id(unsigned int idx) const
{
  vpro_storage_sptr ptr = get_input(idx);
  if(!ptr)
    return typeid(void);
  return ptr->type_id();
}
...
bool
vpro_process::set_output(unsigned int idx, const vpro_storage_sptr& data)
{
...
unsigned int
vpro_process::num_observers(unsigned int idx) const
{
  typedef vcl_map<unsigned int, vcl_set<vpro_observer_sptr> >::const_iterator Itr;
  Itr result = output_observers_.find(idx);
  if(result == output_observers_.end())
    return 0;
...
vpro_process::observed_outputs() const
{
  vcl_set<unsigned int> observed;
  typedef vcl_map<unsigned int, vcl_set<vpro_observer_sptr> >::const_iterator OItr;
  for(OItr obs = output_observers_.begin(); 
      obs != output_observers_.end(); ++obs)
